...
module.exports = {
    ...
    getAllClients: (req, res) => {
        sequelize.query(`SELECT * FROM cc_users AS u JOIN cc_clients AS c ON c.user_id = u.user_id WHERE u.user_id = ${userId};`)
        .then(dbRes => res.status(200).send(dbRes[0]))
        .catch(err => console.log(err))
    ...
    completeAppointment: (req, res) => {
       let {apptId} = req.body
        ...
        sequelize.query(`UPDATES cc_appointments SET completed = true 
        WHERE appt_id = ${apptId};`)
        .then(dbRes => res.status(200).send(dbRes[0]))
        .catch(err => console.log(err))
